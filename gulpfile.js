...
gulp.task('styles:base', function() {
    return gulp.src(['./src/web/assets/scss.base/base.scss'])
        .pipe(gulpsass({
            outputStyle: 'expanded',
            //outputStyle: 'compressed', 
            sourceComments: 'map',
            includePaths: [
                './src/web/assets/scss.base/theme',
...
gulp.task('styles:ap', function() {
    return gulp.src(['./src/web/assets/scss.ap/ap.scss'])
        .pipe(gulpsass({
            outputStyle: 'expanded',
            //outputStyle: 'compressed', 
            sourceComments: 'map',
...
gulp.task('styles', function(done) {
    runSequence('styles:base', 'styles:ap', done);
});

gulp.task('html', function() {
    return gulp.src(['./src/web/**/*', '!./src/web/assets/**',
            '!./src/web/vendor{,/**}'
        ])
...
gulp.task('font:sass', function() {
    return gulp.src(['./src/web/assets/font/appfont.scss'])
        .pipe(gulpsass({
            outputStyle: 'expanded',
            //outputStyle: 'compressed', 
            sourceComments: 'map',
...
gulp.task('bootstrap:js', function() {
    ...
    for (var i = 0; i < files.length; i++) {
        paths.push(
            './src/web/vendor/bootstrap-sass-official/vendor/assets/javascripts/bootstrap/' +
            files[i]);
    }
...
gulp.task('bootstrap:font', function() {
    return gulp.src([
            './src/web/vendor/bootstrap-sass-official/vendor/assets/fonts/bootstrap/**'
        ])
        .pipe(gulp.dest('./dist/web/assets/fonts'));
});
...
gulp.task('watch', function() {
    ...
    gulp.watch(['./src/web/**/*', '!./src/assets/**',
        '!./src/web/vendor{,/**}'
    ], ['html']);
    gulp.watch(['./src/web/assets/scss.base/**/*.scss',
        './src/web/assets/scss.ap/**/*.scss'
    ], ['styles']);
    gulp.watch(['./src/web/assets/img/**'], ['img']);
});

gulp.task('build:web', function(done) {
    runSequence('bower', ['styles', 'html', 'img', 'font:font', 'font:sass',
        'bootstrap:js', 'bootstrap:font', 'fontawesome:font',
        'jquery:js', 'moment:js', 'modernizr:js', 'js'
    ], done);
});
...
gulp.task('build:mobile:ios', function(done) {
    runSequence('install:mobile', 'install:mobile:ios',
        'install:mobile:plugins', 'copy:mobile:www', 'copy:mobile:ios',
        'build:cordova', done);
});

gulp.task('build:mobile:android', function(done) {
    runSequence('install:mobile', 'install:mobile:android',
        'install:mobile:plugins', 'copy:mobile:www', 'copy:mobile:android',
        'build:cordova', done);
});

gulp.task('build:mobile:all', function(done) {
    runSequence('install:mobile', 'install:mobile:ios',
        'install:mobile:android', 'install:mobile:plugins',
        'copy:mobile:www', 'copy:mobile:ios', 'copy:mobile:android',
        'build:cordova', done);
});
...
gulp.task('update:mobile:android', function(done) {
    runSequence('copy:mobile:www', 'copy:mobile:android', 'build:cordova',
        done);
});
